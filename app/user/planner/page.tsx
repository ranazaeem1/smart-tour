...
function PlannerContent() {
  ...
  const generate = () => {
    if (!dest || !days || !group || !startDate) return;
    setLoading(true);
    ...
    <div className="animate-fade space-y-10 pb-20" role="main">
      ...
      {showRecommendation && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-xl animate-fade" onClick={() => setShowRecommendation(false)} />
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-[var(--radius-xl)] p-8 md:p-12 shadow-2xl w-full max-w-[650px] relative z-10 animate-fade-in-up overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-10 relative z-10">
              <div>
                ...
                <h3 className="text-3xl font-black text-[var(--foreground)] tracking-tighter">Tailored Expeditions</h3>
                <p className="text-[var(--muted-foreground)] font-bold text-xs uppercase tracking-widest mt-2">Matching {dest} tours under {formatPKR(budget)}</p>
              </div>
              <button onClick={() => setShowRecommendation(false)} className="w-11 h-11 flex items-center justify-center hover:bg-[var(--muted)] rounded-xl transition-colors text-[var(--muted-foreground)]" aria-label="Close recommendations">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="space-y-6 max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar relative z-10">
              {searchingTours ? (
                <div className="py-20 flex flex-col items-center justify-center">
                  ...
                  <p className="text-[var(--muted-foreground)] font-black uppercase tracking-widest text-[10px]">Scanning Global Database...</p>
                </div>
              ) : matchingTours && matchingTours.length > 0 ? (
                <div className="space-y-4">
                  <div className="p-5 bg-tint-green rounded-[var(--radius-lg)] border-emerald-100/50 flex items-center gap-4 animate-fade">
                    <div className="w-10 h-10 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-600">
                      <CheckCircle2 size={20} />
                    </div>
                    <p className="text-emerald-800 dark:text-emerald-400 text-[11px] font-black uppercase tracking-widest">Found {matchingTours.length} optimal matches.</p>
                  </div>
                  {matchingTours.map((tour, idx) => (
                    <div key={tour.id} className="group flex gap-6 p-6 bg-[var(--muted)] hover:bg-[var(--card)] hover:shadow-xl border border-transparent hover:border-[var(--border)] rounded-[var(--radius-xl)] transition-all duration-500 animate-fade" style={{ animationDelay: `${idx * 100}ms` }}>
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        <div>
                          <h4 className="text-lg font-black text-[var(--foreground)] group-hover:text-emerald-500 transition-colors leading-tight">{tour.title}</h4>
                          <div className="flex items-center gap-2 mt-2">
                            <span className="text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-widest">{tour.companies?.name}</span>
                            <span className="w-1 h-1 rounded-full bg-[var(--border)]" />
                            <div className="flex items-center gap-1">
                              <Star size={10} className="text-amber-400 fill-amber-400" />
                              <span className="text-[11px] font-black text-[var(--foreground)]">{tour.rating}</span>
                            </div>
                          </div>
                        </div>
                ...
                <div className="text-center py-20 px-10">
                  <div className="w-20 h-20 bg-[var(--muted)] rounded-[32px] flex items-center justify-center text-[var(--muted-foreground)] mx-auto mb-6 shadow-inner border border-[var(--border)]">
                    <Compass size={40} />
                  </div>
                  <h4 className="text-2xl font-black text-[var(--foreground)] mb-2 tracking-tight">No Matching Expeditions</h4>
                  <p className="text-[var(--muted-foreground)] font-medium text-sm leading-relaxed max-w-sm mx-auto">
                    We couldn't find active tours in {dest} within your budget constraints.
                  </p>
            ...
            <div className="mt-10 pt-8 border-t border-[var(--border)] flex items-center justify-center relative z-10">
              <p className="text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-widest">Real-time operator synchronization enabled.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
        <div>
          <p className="text-[var(--muted-foreground)] text-[10px] font-black uppercase tracking-[0.2em] mb-2">Algorithm-Driven Planning</p>
          <h1 className="m-0 tracking-tighter text-4xl">Smart Itinerary</h1>
        </div>
        <nav className="flex gap-2 p-2 bg-[var(--muted)] rounded-[var(--radius-lg)] border border-[var(--border)]" aria-label="Planner steps">
          {["Preferences", "Review", "Itinerary"].map((l, i) => (
            <button 
              key={l} 
              className={`px-6 py-3 rounded-[var(--radius-md)] text-[11px] font-black uppercase tracking-widest transition-all duration-300 ${step === i + 1 ? "bg-[var(--card)] text-[var(--foreground)] shadow-lg" : "text-[var(--muted-foreground)] hover:text-[var(--foreground)]"}`} 
              onClick={() => setStep(i + 1)}
      ...
      {step === 1 && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-10">
            <section className="card-premium">
              <div className="flex items-center gap-4 mb-10">
                ...
                <div>
                  <h2 className="text-xl font-black text-[var(--foreground)] m-0">Expedition Objectives</h2>
                  <p className="text-[10px] font-bold text-[var(--muted-foreground)] uppercase tracking-widest mt-1">Define your core mission parameters</p>
                </div>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label htmlFor="dest" className="text-[11px] font-black text-[var(--muted-foreground)] uppercase tracking-widest px-1">Destination Region</label>
                ...
                <div className="space-y-3">
                  <label htmlFor="budget" className="text-[11px] font-black text-[var(--muted-foreground)] uppercase tracking-widest px-1 flex justify-between">
                    Budget Threshold <span className="text-emerald-500">{formatPKR(budget)}</span>
                  </label>
                  <div className="py-2">
                    <input id="budget" type="range" min={10000} max={200000} step={5000} value={budget} onChange={e => setBudget(Number(e.target.value))} className="w-full h-2 bg-[var(--muted)] rounded-lg appearance-none cursor-pointer accent-emerald-500" />
                    <div className="flex justify-between mt-3 text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-tighter">
                      <span>10K</span><span>50K</span><span>100K</span><span>150K</span><span>200K</span>
                    </div>
                  </div>
                </div>
                <div className="space-y-3">
                  <label htmlFor="days" className="text-[11px] font-black text-[var(--muted-foreground)] uppercase tracking-widest px-1">Duration (Days)</label>
                ...
                <div className="space-y-3">
                  <label htmlFor="group" className="text-[11px] font-black text-[var(--muted-foreground)] uppercase tracking-widest px-1">Unit Size (Personnel)</label>
                ...
                <div className="space-y-3 md:col-span-2">
                  <label htmlFor="startDate" className="text-[11px] font-black text-[var(--muted-foreground)] uppercase tracking-widest px-1">Deployment Date</label>
                  <input id="startDate" className="input !py-4 font-black" type="date" value={startDate} onChange={e => setStartDate(e.target.value)} />
                </div>
              </div>
            </section>
            ...
            <section className="card-premium">
              <div className="flex items-center gap-4 mb-10">
                ...
                <div>
                  <h2 className="text-xl font-black text-[var(--foreground)] m-0">Regional Interests</h2>
                  <p className="text-[10px] font-bold text-[var(--muted-foreground)] uppercase tracking-widest mt-1">Configure your experience filters</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-3">
                {INTERESTS.map(i => (
                  <button key={i} onClick={() => toggleInterest(i)}
                    className={`px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all duration-300 border ${interests.includes(i) ? "bg-slate-900 border-slate-900 text-white shadow-xl dark:bg-emerald-500 dark:border-emerald-500" : "bg-[var(--card)] border-[var(--border)] text-[var(--muted-foreground)] hover:text-[var(--foreground)] hover:bg-[var(--muted)]"}`}>
                    {i}
                  </button>
                ))}
              </div>
          ...
          <aside className="space-y-10">
            <div className="card-premium bg-slate-950 border-none relative overflow-hidden group shadow-2xl">
              ...
              <h2 className="text-xl font-black text-white mb-10 relative z-10 flex items-center gap-3">
              ...
              <div className="space-y-6 relative z-10">
                {[
                  ...
                  <div key={r.label} className="flex justify-between items-center py-4 border-b border-white/5">
                    ...
                    <span className="text-sm font-black text-slate-200">{r.value}</span>
                  </div>
                ))}
                <div className="pt-6">
            ...
            <div className="card-premium !p-8">
              <div className="flex items-center gap-4 mb-8">
                ...
                <h2 className="text-lg font-black text-[var(--foreground)] m-0">Safety Analysis</h2>
              </div>
              {safetyInfo ? (
                <div className="space-y-6">
                  <div className="flex items-center gap-5">
                    <div className="w-14 h-14 rounded-2xl bg-[var(--muted)] flex items-center justify-center border border-[var(--border)] shadow-inner">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-[var(--muted-foreground)] font-bold uppercase tracking-widest mt-1">Integrity Confirmed</p>
                    </div>
                  </div>
                  <div className="p-5 bg-tint-green rounded-[var(--radius-lg)] border border-emerald-100/30 flex items-start gap-4">
            ...
            <div className="space-y-4">
              ...
              {!isFormValid && (
                <div className="flex items-center justify-center gap-2 px-4 py-2 bg-rose-500/10 border border-rose-500/20 rounded-xl">
                   ...
                   <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest">Incomplete parameters detected</p>
                </div>
              )}
            </div>
          </aside>
          ...
          <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="badge badge-emerald">Strategy Optimized</span>
                <span className="text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-widest">{days} Days in {dest}</span>
              </div>
              <h2 className="text-4xl font-black text-[var(--foreground)] tracking-tighter m-0">Expedition Blueprint</h2>
            </div>
            <div className="flex flex-wrap gap-4">
              <button className="btn btn-secondary !px-6 py-4 flex items-center gap-2 group" onClick={() => {
                const lines = [`SmartTour Strategy — ${days}-Day ${dest} Expedition`, `Group: ${group} People | Budget: ${formatPKR(budget)} pp | Total: ${formatPKR(budget * Number(group))}`, '', ...dynamicItinerary.map(day => [`Day ${day.day}: ${day.title}`, `  Locales: ${day.places.join(', ')}`, `  Quarters: ${day.accommodation}`, `  Rations: ${day.meals.join(', ')}`, `  Climate: ${day.weather}`, ''].join('\n'))].join('\n');
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <section className="lg:col-span-2 space-y-10" aria-label="Daily Objectives">
              {dynamicItinerary.map((day, idx) => (
                <article key={day.day} className="card-premium !p-10 hover:shadow-2xl transition-all duration-500 animate-fade group" style={{ animationDelay: `${idx * 100}ms` }}>
                  <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-10">
                    <div className="flex items-center gap-8">
                      <div className="w-16 h-16 rounded-[24px] bg-slate-950 flex items-center justify-center text-white font-black text-2xl shadow-xl shrink-0 group-hover:rotate-3 transition-transform">
                        {day.day < 10 ? `0${day.day}` : day.day}
                      </div>
                      <div>
                        <h3 className="text-2xl font-black text-[var(--foreground)] m-0 tracking-tight">{day.title}</h3>
                        <div className="flex items-center gap-2 mt-2">
                          <Clock size={14} className="text-emerald-500" />
                          <span className="text-[11px] font-black text-[var(--muted-foreground)] uppercase tracking-[0.1em]">Active Window: {day.travelTime}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 px-6 py-3 bg-[var(--muted)] border border-[var(--border)] rounded-2xl shadow-sm">
                      {day.weatherIcon}
                      <span className="text-[12px] font-black text-[var(--foreground)] uppercase tracking-tighter">{day.weather}</span>
                    </div>
                  </div>
                  
                  <div className="flex flex-wrap gap-3 mb-10">
                    {day.places.map(p => (
                      <span key={p} className="badge badge-slate !px-5 !py-2.5 !text-[11px] hover:bg-slate-900 hover:text-white dark:hover:bg-emerald-500 transition-colors cursor-default">
                        <MapPin size={12} className="mr-2" /> {p}
                      </span>
                    ))}
                  </div>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-10 border-t border-[var(--border)]">
                    <div className="flex items-center gap-5 p-4 bg-[var(--muted)]/50 rounded-2xl border border-[var(--border)]">
                      <div className="w-12 h-12 rounded-xl bg-[var(--card)] flex items-center justify-center text-[var(--muted-foreground)] shadow-sm">
                        <Hotel size={22} />
                      </div>
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-[var(--muted-foreground)] mb-1">Accommodation</p>
                        <p className="text-sm font-black text-[var(--foreground)]">{day.accommodation}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-5 p-4 bg-[var(--muted)]/50 rounded-2xl border border-[var(--border)]">
                      <div className="w-12 h-12 rounded-xl bg-[var(--card)] flex items-center justify-center text-[var(--muted-foreground)] shadow-sm">
                      ...
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-[var(--muted-foreground)] mb-1">Rations</p>
                        <p className="text-sm font-black text-[var(--foreground)]">{day.meals.join(", ")}</p>
                      </div>
                    </div>
                  </div>
                </article>
            ...
            <aside className="space-y-10">
              <section className="card-premium">
                <div className="flex items-center gap-4 mb-10">
                  ...
                  <h3 className="text-xl font-black text-[var(--foreground)] m-0 tracking-tight">Resource Allocation</h3>
                </div>
                <div className="space-y-10">
                  ...
                  <div className="pt-8 border-t border-[var(--border)]">
                    <div className="flex justify-between items-center">
                      <p className="text-[11px] font-black text-[var(--muted-foreground)] uppercase tracking-widest">Aggregate (Per Personnel)</p>
                      <p className="text-3xl font-black text-emerald-500 tracking-tighter">{formatPKR(budget)}</p>
                    </div>
                  </div>
                </div>
              </section>

              <section className="card-premium bg-slate-950 border-none relative overflow-hidden shadow-2xl">
                ...
                <div className="space-y-4 relative z-10">
                  {[
                    { color: "text-white shadow-[0_0_10px_#FFFFFF]", msg: "Atmospheric Prediction: Light precipitation on Day 3. Standard gear sufficient." },
              ...
              <section className="card-premium">
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-10 h-10 rounded-xl bg-[var(--muted)] flex items-center justify-center text-[var(--foreground)] border border-[var(--border)]">
                    <Users size={18} />
                  </div>
                  <h3 className="text-lg font-black text-[var(--foreground)] tracking-tight">Expedition Unit</h3>
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between items-center py-4 border-b border-[var(--border)]">
                    <span className="text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-widest">Unit Strength</span>
                    <span className="text-sm font-black text-[var(--foreground)]">{group} Personnel</span>
                  </div>
                  <div className="flex justify-between items-center py-4 border-b border-[var(--border)]">
                    <span className="text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-widest">Base Rate</span>
                    <span className="text-sm font-black text-emerald-500">{formatPKR(budget)}</span>
                  </div>
                  <div className="pt-6">
                    <p className="text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-widest mb-1">Projected Deployment Cost</p>
                    <p className="text-3xl font-black text-[var(--foreground)] tracking-tighter">{formatPKR(budget * Number(group))}</p>
                  </div>
                </div>
              </section>
            </aside>
      ...
      {/* ── Step 2: Review ── */}
      {step === 2 && (
        <section className="card-premium max-w-[800px] mx-auto animate-fade p-12 md:p-20 relative overflow-hidden">
          ...
          <div className="text-center mb-16 relative z-10">
            ...
            <h2 className="text-4xl font-black text-[var(--foreground)] m-0 tracking-tighter uppercase">Mission Validation</h2>
            <p className="text-[var(--muted-foreground)] font-bold uppercase tracking-widest text-[11px] mt-4">Finalizing Expedition Parameters</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
            {[
              ...
              <div key={r.label} className="p-8 bg-[var(--muted)] border border-[var(--border)] rounded-[var(--radius-xl)] group hover:bg-[var(--card)] hover:shadow-2xl transition-all duration-500">
                <div className="flex items-center gap-6">
                  <div className="w-14 h-14 rounded-2xl bg-[var(--card)] flex items-center justify-center shadow-lg border border-[var(--border)] group-hover:scale-110 transition-transform">
                    {r.icon}
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-widest leading-none mb-2">{r.label}</p>
                    <p className="text-sm font-black text-[var(--foreground)] group-hover:text-emerald-500 transition-colors">{r.value}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="mt-16 flex flex-col gap-6 relative z-10">
            ...
            <button onClick={() => setStep(1)} className="text-[11px] font-black text-[var(--muted-foreground)] uppercase tracking-[0.2em] hover:text-[var(--foreground)] transition-colors py-2 text-center">Modify Mission Parameters</button>
          </div>
        </section>
      )}
    </div>
...
export default function PlannerPage() {
  return (
    <Suspense fallback={<div className="flex flex-col items-center justify-center h-[60vh] space-y-6"><div className="loading-spinner h-12 w-12"/><p className="text-[var(--muted-foreground)] font-black uppercase tracking-widest text-[10px]">Initializing AI Kernel...</p></div>}>
      <PlannerContent />
    </Suspense>
  );
}
