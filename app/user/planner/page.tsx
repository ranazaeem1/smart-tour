          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-8 md:p-12 shadow-2xl w-full max-w-[650px] relative z-10 animate-fade-in-up overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-10 relative z-10">
              <div>
                ...
                <p className="text-slate-500 font-bold text-xs uppercase tracking-widest mt-2">Matching {dest} tours under {formatPKR(budget)}</p>
              </div>
            ...
            <div className="space-y-6 max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar relative z-10">
              {searchingTours ? (
                <div className="py-20 flex flex-col items-center justify-center">
                  ...
                  <p className="text-slate-500 font-black uppercase tracking-widest text-[10px]">Scanning Global Database...</p>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <section className="lg:col-span-2 space-y-10" aria-label="Daily Objectives">
              {dynamicItinerary.map((day, idx) => (
                <article key={day.day} className="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-sm hover:shadow-xl transition-all duration-500 animate-fade group" style={{ animationDelay: `${idx * 100}ms` }}>
                    ...
                    <div className="flex items-center gap-8">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span className="text-[11px] font-black text-slate-500 uppercase tracking-[0.1em]">Active Window: {day.travelTime}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 px-6 py-3 bg-slate-50 border border-slate-200 rounded-2xl shadow-sm">
                      {day.weatherIcon}
                      <span className="text-[12px] font-black text-slate-950 uppercase tracking-tighter">{day.weather}</span>
                    </div>
                  </div>
                  
                  <div className="flex flex-wrap gap-3 mb-10">
                    {day.places.map(p => (
                      <span key={p} className="badge badge-slate !px-5 !py-2.5 !text-[11px] hover:bg-slate-100 hover:text-slate-950 dark:hover:bg-emerald-500 transition-colors cursor-default">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-10 border-t border-slate-200">
                    <div className="flex items-center gap-5 p-4 bg-slate-50/50 rounded-2xl border border-slate-200">
                      ...
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Accommodation</p>
                        <p className="text-sm font-black text-slate-950">{day.accommodation}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-5 p-4 bg-slate-50/50 rounded-2xl border border-slate-200">
                      <div className="w-12 h-12 rounded-xl bg-white flex items-center justify-center text-slate-500 shadow-sm">
                      ...
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Rations</p>
                        <p className="text-sm font-black text-slate-950">{day.meals.join(", ")}</p>
                      </div>
                    </div>
                  </div>
                </article>
                ...
                <div className="space-y-10">
                  {BUDGET_BREAKDOWN.map(item => (
                    <div key={item.name} className="group">
                      <div className="flex justify-between items-end mb-3">
                        ...
                        <span className="text-sm font-black text-slate-950">{formatPKR(item.amount)}</span>
                      </div>
                  ...
                  <div className="pt-8 border-t border-slate-200">
                    <div className="flex justify-between items-center">
                      <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Aggregate (Per Personnel)</p>
