...
export default function TopNav({ title, onMenuClick, hideMobileMenuButton = false }: TopNavProps) {
  ...
  return (
    <header className="sticky top-0 z-[100] flex min-h-[72px] items-center justify-between gap-3 bg-[var(--card)]/80 backdrop-blur-xl px-3 py-3 sm:px-4 md:px-6 lg:px-8 md:min-h-[80px] border-b border-[var(--border)] shadow-[var(--shadow-sm)] transition-all duration-300">
      {/* Left Section: Mobile Menu & Title */}
      <div className="flex min-w-0 flex-1 items-center gap-3 sm:gap-4">
        {!hideMobileMenuButton && (
          <button 
            onClick={onMenuClick}
            className="lg:hidden shrink-0 p-2.5 bg-[var(--muted)] text-[var(--muted-foreground)] rounded-xl hover:bg-[var(--border)] active:scale-95 transition-all"
        ...
        <div className="animate-fade min-w-0">
          <p className="hidden min-[420px]:inline-flex text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-[0.2em] mb-1 items-center gap-1.5">
      ...
      <div className="flex shrink-0 items-center gap-2 sm:gap-3 md:gap-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3 md:gap-5">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {isDropdownOpen && (
              <div 
                className="absolute right-0 mt-3 w-[min(16rem,calc(100vw-1.5rem))] bg-[var(--card)] border border-[var(--border)] rounded-[var(--radius-lg)] shadow-[var(--shadow-xl)] py-3 z-[110] animate-fade-in-up"
                role="menu"
